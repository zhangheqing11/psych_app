...
def get_assessment_prompt_text():
	...
	1.2自尊管理：从打击中恢复原状的能力。
	包括自尊的脆弱性（水平高低、稳定性）、应对挫败自尊的方式（自恋自大或自我挫败/受虐，而更健康的方式是直面问题 而不是困在自我中）、利用他人调节自尊。自尊是对自己的尊敬和或欣赏，自尊问题使我们变得无法承受情感和焦虑，无法现实评价能力和局-限，无法控制我们的冲动，无法放松，等等。
...
# --- 绑定码验证和来访者添加API (前端需要的新端点) ---
@app.route('/api/verify-binding-code', methods=['POST'])
def verify_binding_code():
    data = request.get_json()
    binding_code = data.get('binding_code')
    
    if not binding_code:
        return jsonify({"message": "请提供绑定口令"}), 400
    
    all_data = read_data()
    
    # 查找具有此绑定码的来访者
    client_with_code = next((c for c in all_data.get('counselor_data', {}).get('clients', []) 
                           if c.get('binding_code') == binding_code), None)
    
    if not client_with_code:
        return jsonify({"message": "绑定口令无效或已过期"}), 404
    
    # 检查该来访者是否已被分配给其他咨询师
    for counselor in all_data.get('counselor_data', {}).get('counselors', []):
        if client_with_code.get('id') in counselor.get('assignedClientIds', []):
            return jsonify({"message": "该来访者已被其他咨询师管理"}), 409
    
    # 返回来访者基本信息（不包含敏感信息）
    client_info = {
        "id": client_with_code.get('id'),
        "username": client_with_code.get('username'),
        "name": client_with_code.get('name', client_with_code.get('username', '')),
        "existing_data": bool(client_with_code.get('age') or client_with_code.get('contact'))
    }
    
    return jsonify({"message": "绑定口令验证成功", "client": client_info}), 200

@app.route('/api/add-client-to-counselor', methods=['POST'])
def add_client_to_counselor():
    data = request.get_json()
    counselor_username = data.get('counselor_username')
    binding_code = data.get('binding_code')
    client_data = data.get('client_data')
    
    if not all([counselor_username, binding_code, client_data]):
        return jsonify({"message": "缺少必要参数"}), 400
    
    all_data = read_data()
    
    # 验证咨询师存在
    counselor_index = next((i for i, c in enumerate(all_data.get('counselor_data', {}).get('counselors', [])) 
                          if c.get('username') == counselor_username), -1)
    if counselor_index == -1:
        return jsonify({"message": "咨询师不存在"}), 404
    
    # 再次验证绑定码并找到来访者
    client_index = next((i for i, c in enumerate(all_data.get('counselor_data', {}).get('clients', [])) 
                        if c.get('binding_code') == binding_code), -1)
    
    if client_index == -1:
        return jsonify({"message": "绑定口令无效"}), 404
    
    existing_client = all_data['counselor_data']['clients'][client_index]
    
    # 检查来访者是否已被其他咨询师分配
    for counselor in all_data.get('counselor_data', {}).get('counselors', []):
        if existing_client.get('id') in counselor.get('assignedClientIds', []):
            return jsonify({"message": "该来访者已被其他咨询师管理"}), 409
    ...
    updated_client = {
        **existing_client,
        **client_data,
        'id': existing_client['id'],  # 保持原有ID
        'username': existing_client['username'],  # 保持原有用户名
        'binding_code': existing_client['binding_code'],  # 保持绑定码
        'sessions': existing_client.get('sessions', []),  # 保持会话记录
        'joinDate': existing_client.get('joinDate')  # 保持加入日期
    }
    
    all_data['counselor_data']['clients'][client_index] = updated_client
    
    # 将来访者分配给当前咨询师
    if 'assignedClientIds' not in all_data['counselor_data']['counselors'][counselor_index]:
        all_data['counselor_data']['counselors'][counselor_index]['assignedClientIds'] = []
    
    if updated_client['id'] not in all_data['counselor_data']['counselors'][counselor_index]['assignedClientIds']:
        all_data['counselor_data']['counselors'][counselor_index]['assignedClientIds'].append(updated_client['id'])
    
    write_data(all_data)
    
    return jsonify({
        "message": "来访者已成功添加到您的管理列表",
